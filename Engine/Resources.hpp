...
namespace Engine {
    ...
    public:
        ...
        /// <returns>The smart pointer of the bitmap.</returns>
        std::shared_ptr<ALLEGRO_BITMAP> GetBitmap(std::string name, int width, int height, std::string pathPrefix = bitmapPathPrefix);
        std::shared_ptr<ALLEGRO_BITMAP> GetSVG(std::string name, int width = -1, int height = -1, std::string pathPrefix = bitmapPathPrefix);
        /// <summary>
